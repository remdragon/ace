...
function createTree(p_div,p_backColor,p_contextMenu) {
	var tree = {
		...
		// p_prefix: optional text that will appear before the icon
		createNode: function(p_text,p_expanded, p_icon, p_parentNode,p_tag,p_contextmenu, p_prefix) {
			v_tree = this;
			node = {
				...
				createChildNode: function(p_text,p_expanded,p_icon,p_tag,p_contextmenu,p_prefix) { return v_tree.createNode(p_text,p_expanded,p_icon,this,p_tag,p_contextmenu,p_prefix); }
			}
			...
			if (this.rendered) {
				if (p_parentNode==undefined) {
					...
					if (p_parentNode.childNodes.length==0) {
						...
						v_img.src = '/aimara/images/collapse.png';
						v_img.id = 'toggle_off';
						}
						else {
							...
							v_img = p_parentNode.elementLi.getElementsByTagName("img")[0];
							v_img.style.visibility = "visible";
							v_img.src = '/aimara/images/expand.png';
							v_img.id = 'toggle_on';
						}
					}
      ...
      this.adjustLines(document.getElementById(this.name),true);
		...
		drawNode: function(p_ulElement,p_node) {
			...
			p_node.elementSpan = v_span

			var v_exp_col = null;

			if (p_node.childNodes.length == 0) {
				v_exp_col = createImgElement('toggle_off','exp_col','/aimara/images/collapse.png');
				...
				if (p_node.expanded) {
					...
					v_exp_col = createImgElement('toggle_on','exp_col','/aimara/images/expand.png');
				}
			}
			...
			if ( p_node.prefix != undefined )
				v_span.appendChild ( document.createTextNode ( p_node.prefix ) )
		...
		expandNode: function(p_node) {
			if (p_node.childNodes.length>0 && p_node.expanded==false) {
				...
				img.id="toggle_off";
				img.src = '/aimara/images/collapse.png';
		...
		collapseNode: function(p_node) {
			if (p_node.childNodes.length>0 && p_node.expanded==true) {
				...
				img.id="toggle_on";
				img.src = '/aimara/images/expand.png';
				elem_ul = img.parentElement.getElementsByTagName("ul")[0];
		...
		selectNode: function(p_node) {
			...
			if (this.nodeSelectedEvent!=undefined)
				this.nodeSelectedEvent(p_node)
		},
		///// Deleting node
		// p_node: Reference to the node;
				...
				if (p_node.contextMenu!=undefined) {
					...
					for (var i=0; i<v_menu.elements.length; i++) (function(i){
						...
						if (v_menu.elements[i].submenu!=undefined) {
							...
							v_span_more.appendChild(createImgElement(null,'menu_img','/aimara/images/right.png'));
